...
class CommandFunctions {
    ...
    public function __construct(Avast $plugin){
        $this->plugin = $plugin;
    }

    /**
     * Function that is executed when /Avast about is invoked.
    ...
    public function cmdEmpty(CommandSender $sender) {
        $sender->sendMessage($this->plugin->msg("Avast Command List"));
        if($sender->hasPermission("Avast.command.Avast")) {
            if($this->plugin->cache instanceof SimpleCache) {
              if($sender->hasPermission("Avast.command.clearcache")) {
                  $sender->sendMessage($this->plugin->msg("/Avast clearcache"));
              }
              if($sender->hasPermission("Avast.command.clearip")) {
                  $sender->sendMessage($this->plugin->msg("/Avast clearip <ipv4 address>"));
              }
            }
            if($sender->hasPermission("Avast.command.lookup")) {
                $sender->sendMessage($this->plugin->msg("/Avast lookup <ipv4 address>"));
            }
            if($sender->hasPermission("Avast.command.subnet")) {
                $sender->sendMessage($this->plugin->msg("/Avast subnet <ban/unban> <ipv4 address/subnet>"));
            }
            if($sender->hasPermission("Avast.command.country")) {
                $sender->sendMessage($this->plugin->msg("/Avast country <add/remove> <country code>"));
            }
            if($sender->hasPermission("Avast.command.about")) {
                $sender->sendMessage($this->plugin->msg("/Avast about"));
            }
        } else {
    ...
    /**
     * Function that is executed when /Avast clearcache is invoked
    ...
    /**
     * Function that is executed when /Avast clearip <ip> is invoked
    ...
    /**
     * Function that is executed when /Avast lookup <ip> is invoked
    ...
    public function cmdAbout(CommandSender $sender) {
        $sender->sendMessage($this->plugin->msg("Avast v" . $this->plugin->getDescription()->getVersion()));
    ...
    */
    public function cmdSubnet(CommandSender $sender, $mode, $ip) {
      ...
      if(($mode === "ban") || ($mode === "unban")) {
        ...
        $sender->sendMessage($this->plugin->msg("Usage: /Avast subnet <ban/unban> <ipv4 address/subnet>"));
      }
    }

    /**
    * Function that is executed when /Avast country is invoked
    ...
    public function cmdCountry(CommandSender $sender, $mode, $countryCode) {
      ...
      if(array_key_exists($countryCode,$codesList)) {
        if($mode === "add") {
          ...
          $sender->sendMessage($this->plugin->msg("Usage: /Avast country <add/remove> <country code>"));
        }
      } else {
